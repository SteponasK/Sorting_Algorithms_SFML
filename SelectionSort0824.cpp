...
int runSelectionSort() { 

    sf::RenderWindow window(sf::VideoMode(960, 540), "Selection Sort");
    ...
    int minimum_index = 0; // Moving Bar last Index?
    ...
    for (int i = 0; i < infoTextCount; ++i)
    {
        ...
        infoText[1].setString("Selection sort is a simple algorithm that");
        infoText[2].setString("works by repeatedly selecting the smallest");
        infoText[3].setString("element from the unsorted portion of the list ");
        infoText[4].setString("and moving it to the sorted portion of the list. ");

        infoBackground.setFillColor(sf::Color(88, 69, 97, 127));
        infoBackground.setSize(sf::Vector2f(infoText[3].getGlobalBounds().width * 1.11, 240));
        ...
        while (window.isOpen())
        {
            ...
            for (int i = 0; i < lines; i++)
            {
                if (line[minimum_index].height > line[i].height && i >= beggining && i == iteration) // find lowest
                {
                    minimum_index = i;
                }        
                ...
                if ((iteration == i) && (iteration >= beggining) && (!sorted)) rectangle.setFillColor(sf::Color(131, 130, 119));  // Moving bar - Color
                if ((minimum_index == i) && (iteration >= beggining) && (!sorted)) rectangle.setFillColor(sf::Color(139,0,0)); // Lowest number index - Color
